...
dbimperial = SQL("sqlite:///databases/imperialnet.db")
db = SQL("sqlite:///databases/users.db")
dbholocron1 = SQL("sqlite:///databases/hint.db")
dbvader = SQL("sqlite:///databases/vader.db")
darkdb = SQL("sqlite:///databases/hint.db")
commandb = SQL("sqlite:///databases/admins.db")
...
@app.route("/ImperialWebscraper")
@login_required
def ImperialWebscraper():
    #TODO implement web scraper for ImperialNet
    return render_template("rebel/ImperialWebscraper")
...
def dark_holocron():
    ...
    if request.method == 'POST':
        ...
        query = dbholocron1.execute(f"SELECT id FROM holocron WHERE username = '{username}' AND pass = '{password}'")
...
@app.route("/darkarchive/palpatineadmin")
def admin():
